...
//void MultiplyMatrix(int[,] FirstGetMatrix, int[,] SecondGetMatrix, int[,] resultMatrix)
//{
 //   for (int i = 0; i < resultMatrix.GetLength(0); i++)
  //  {
  //      for (int j = 0; j < resultMatrix.GetLength(1); j++)
  //      {
  //          int sum = 0;
  //          for (int k = 0; k < FirstGetMatrix.GetLength(1); k++)
  //          {
  //              sum += FirstGetMatrix[i, k] * SecondGetMatrix[k, j];
  //          }
...
//int i = 0; int j = 0;
//int[,] resultMatrix1 = new int[i, j];
//MultiplyMatrix(FirstMatrix, SecondMatrix, resultMatrix1);
//Console.WriteLine($"\nПроизведение первой и второй матриц:");
//WriteArray(resultMatrix);
//int[,] ResultMatrix = resultMatrix(rows, columns, 0, 10);
//PrintMatrix(resultMatrix);
int m = 0; int n = 0;
...
for (int i = 0; i < resultMatrix.GetLength(0); i++)
    {
        for (int j = 0; j < resultMatrix.GetLength(1); j++)
        {
            Console.Write(resultMatrix[i, j] + "\t"); // Tab между элементами
        }
        Console.WriteLine();
    }
    */